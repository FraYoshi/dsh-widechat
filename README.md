...
Widens the [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) conversation column to ~98% of the available center-column width, eliminating the ~20% wasted whitespace the shipped 748 px cap introduces.
...
`.wSkVaW_root`, `.Md3f7G_column`, `.uV2eYG_card`, etc. are CSS-Modules-generated class names from `@deepseek-ai/dsh-client-ui-conversation`. Their hashes (the part after the underscore) are recomputed every time that package is rebuilt. Any release of `dsh-client-ui-conversation` can therefore silently break this plugin.

After upgrading `@deepseek-ai/dsh-client-ui-conversation`, check whether the conversation still widens. If it does not, the upstream CSS-module hashes changed; see "Updating" below.
...
1. From `~/.dsh/profiles/web/node_modules/@deepseek-ai/dsh-client-ui-conversation`, find the current CSS-module prefixes. They appear in lines like `var ConversationRoot_module_css_default = { "root": "wSkVaW_root", ... }` in `lib/client.js`.
2. Replace `wSkVaW_*`, `Md3f7G_*`, `uV2eYG_*` in `lib/client.js` with the new prefixes.
3. Bump the version in `package.json` and publish.

A regex search in `dsh-client-ui-conversation/lib/client.js` for `ConversationRoot_module_css_default\s*=\s*{` and the surrounding CSS string is usually enough.
