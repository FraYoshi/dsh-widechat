...
const PRESERVE_GUTTER_WHEN_SIDEBAR_COLLAPSED_FIELD = "preserveGutterWhenSidebarCollapsed";
...
const DEFAULTS = {
	...
	[PRESERVE_GUTTER_WHEN_SIDEBAR_COLLAPSED_FIELD]: true
};
...
function clampGutter(value) {
	if (typeof value !== "number" || !Number.isFinite(value)) return DEFAULTS[CHAT_GUTTER_PCT_FIELD];
...
function WideChatRow({ scope, t }) {
	...
	const gutterPct = clampGutter(readField(section, CHAT_GUTTER_PCT_FIELD, DEFAULTS[CHAT_GUTTER_PCT_FIELD]));
	const statsAlignRaw = readField(section, STATS_ALIGN_FIELD, DEFAULTS[STATS_ALIGN_FIELD]);
	const statsAlign = STATS_ALIGN_VALUES.includes(statsAlignRaw) ? statsAlignRaw : DEFAULTS[STATS_ALIGN_FIELD];
	const preserveGutter = readField(section, PRESERVE_GUTTER_WHEN_SIDEBAR_COLLAPSED_FIELD, DEFAULTS[PRESERVE_GUTTER_WHEN_SIDEBAR_COLLAPSED_FIELD]);
	const setGutter = (0, react.useCallback)((next) => {
		scope.set(CHAT_GUTTER_PCT_FIELD, next);
	}, [scope]);
	const setAlign = (0, react.useCallback)((next) => {
		scope.set(STATS_ALIGN_FIELD, next);
	}, [scope]);
	const setPreserve = (0, react.useCallback)((next) => {
		if (next) scope.set(PRESERVE_GUTTER_WHEN_SIDEBAR_COLLAPSED_FIELD, true);
		else scope.unset(PRESERVE_GUTTER_WHEN_SIDEBAR_COLLAPSED_FIELD);
	}, [scope]);
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
						...
						htmlFor: "dswc-preserve",
						children: t("preserveGutter.label")
					})]
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: "dswc-help",
					children: t("preserveGutter.help")
				})]
			})
		]
	});
...
function resolveConfig(raw, defaults) {
	...
	const preserveGutterInput = section["preserveGutterWhenSidebarCollapsed"] ?? defaults.preserveGutterWhenSidebarCollapsed;
	...
	if (preserveGutterRaw !== preserveGutterInput) fallbacks.push(`${PRESERVE_GUTTER_WHEN_SIDEBAR_COLLAPSED_FIELD}=${JSON.stringify(preserveGutterInput)}`);
	return {
...
`;
	...
	const widthValue = `${100 - config.chatGutterPct * 2}%`;
	return `
:root,
.wSkVaW_root,
[data-phase] {
...
function applyForScope(scopeValue) {
	const resolved = resolveConfig(scopeValue, {
		...
		preserveGutterWhenSidebarCollapsed: DEFAULTS[PRESERVE_GUTTER_WHEN_SIDEBAR_COLLAPSED_FIELD]
	});
...
function apply(ctx) {
	...
	const setSheetFromScope = (snapshot) => {
