...
function readValues(scope) {
	...
	return {
		...
		composerMaxHeightPct: clampNumber(readField(section, COMPOSER_MAX_HEIGHT_PCT_FIELD, DEFAULTS[COMPOSER_MAX_HEIGHT_PCT_FIELD]), 20, 43, DEFAULTS[COMPOSER_MAX_HEIGHT_PCT_FIELD])
	};
}
function WideChatRow({ scope, t }) {
	...
	const setComposerMaxHeight = (0, react.useCallback)((next) => {
		const clamped = clampNumber(next, 20, 43, DEFAULTS[COMPOSER_MAX_HEIGHT_PCT_FIELD]);
...
function resolveConfig(raw, defaults) {
	...
	const composerMaxHeightPctRaw = clampInt(composerMaxHeightPctInput, 20, 43, defaults.composerMaxHeightPct);
...
.uV2eYG_card {
  max-height: ${`${config.composerMaxHeightPct}vh`} !important;
