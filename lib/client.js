 ...
 *   • Right-aligns the session stats strip (token counts, etc.) that ships
 *     centered under the composer.
...
/* Session stats strip: shipped centered, move to right edge. The block keeps
   its outer centering margin so it still lines up with the chat column, and
   the text inside right-aligns. text-overflow:ellipsis + overflow:hidden
   stays in effect — long stats are truncated rather than wrapping. */
.FJxK0a_root {
